...
class Cap_background:
        ...
        def camera_stop(self):
            if self.cam_status == True and self.capture:
        ...
        def getFrame(self):
            print('frame length', len(self.frame))
            while self.frame[-1] is None:
                self.frame.pop(-1)
            gotFrame = self.frame[-1]
            self.frame.pop(-1)
            return gotFrame
...
class DetetionAPP:
    def __init__(self, model_dir):
        ...
        self.cam_status = False
        # self.camera = cv2.VideoCapture('rtsp://192.168.31.88:8554/unicast')
        self.camera = Cap_background('rtsp://192.168.31.88:8554/unicast')
